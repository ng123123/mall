...
//import com.macro.mall.portal.domain.MemberCartPromotionItem;
...
public class MemberPromotionServiceImpl implements MemberPromotionService {
    ...
    @Override
    public List<MemberCartPromotionItem> calcCartPromotion(List<MemberCartItem> cartItemList) {
        //1.先根据productId对CartItem进行分组，以spu为单位进行计算优惠
        Map<Long, List<MemberCartItem>> productCartMap = groupCartItemBySpu(cartItemList);
        //2.查询所有商品的优惠相关信息
        List<PromotionProduct> promotionProductList = getPromotionProductList(cartItemList);
        //3.根据商品促销类型计算商品促销优惠价格
        List<MemberCartPromotionItem> cartPromotionItemList = new ArrayList<>();
        for (Map.Entry<Long, List<MemberCartItem>> entry : productCartMap.entrySet()) {
            Long productId = entry.getKey();
            PromotionProduct promotionProduct = getPromotionProductById(productId, promotionProductList);
            List<MemberCartItem> itemList = entry.getValue();
            Integer promotionType = promotionProduct.getPromotionType();
            if (promotionType == 1) {
                //单品促销
                for (MemberCartItem item : itemList) {
                    MemberCartPromotionItem cartPromotionItem = new MemberCartPromotionItem();
                ...
                if(fullReduction!=null){
                    for (MemberCartItem item : itemList) {
                        ...
                        cartPromotionItem.setPromotionMessage(message);
                        //(商品原价/总价)*满减金额 // 满减以spu为单位，按照sku商品价格占其所属spu总额的比例，均摊到每个sku上 【cnr】
                        PmsSkuStock skuStock= getOriginalPrice(promotionProduct, item.getProductSkuId());
                        BigDecimal originalPrice = skuStock.getPrice();
     ...
     * 查询所有商品的优惠相关信息
     */
    private List<PromotionProduct> getPromotionProductList(List<MemberCartItem> cartItemList) {
        List<Long> productIdList = new ArrayList<>();
        for(MemberCartItem cartItem:cartItemList){
     ...
     * 以spu为单位对购物车中商品进行分组
     */
    private Map<Long, List<MemberCartItem>> groupCartItemBySpu(List<MemberCartItem> cartItemList) {
        Map<Long, List<MemberCartItem>> productCartMap = new TreeMap<>();
        for (MemberCartItem cartItem : cartItemList) {
            List<MemberCartItem> productCartItemList = productCartMap.get(cartItem.getProductId());
     ...
     * 对没满足优惠条件的商品进行处理
     */
    private void handleNoReduce(List<MemberCartPromotionItem> cartPromotionItemList, List<MemberCartItem> itemList,PromotionProduct promotionProduct) {
        for (MemberCartItem item : itemList) {
            MemberCartPromotionItem cartPromotionItem = new MemberCartPromotionItem();
            BeanUtils.copyProperties(item,cartPromotionItem);
     ...
     * 获取购物车中指定商品的数量
     */
    private int getCartItemCount(List<MemberCartItem> itemList) {
        int count = 0;
        for (MemberCartItem item : itemList) {
     ...
     * 获取购物车中指定商品的总价
     */
    private BigDecimal getCartItemAmount(List<MemberCartItem> itemList, List<PromotionProduct> promotionProductList) {
